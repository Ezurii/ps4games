...
if selected_regions:
    df_filtered = df_filtered[['Year', 'Genre', 'Publisher'] + selected_regions]
...
bars = plt.bar(contagem_por_editora.index, contagem_por_editora)
plt.xlabel('Editoras')
plt.ylabel('Número de Jogos')
plt.title('Contagem de Jogos por editora')
...
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width() / 2, yval, round(yval, 1), ha='center', va='bottom', fontsize=5)
...
# Gênero
Year = st.multiselect(
    label="Ano de lançamento no ps4",
    options=df["Year"].unique()
)
...
# Filtra por editora se alguma editora foi selecionada
if Publisher:
    df_filtered = df_filtered[df_filtered["Publisher"].isin(Publisher)]

# Filtra por região se alguma região foi selecionada
if selected_regions:
    df_filtered = df_filtered[['Game', 'Year', 'Publisher'] + selected_regions]
